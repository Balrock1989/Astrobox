...
class Headquarters:
    ...
    def get_roles(self, number_drones, have_gun):
        if have_gun:
            ...
            combats = 0  # number_drones #- transports - collectors
            ...
            Headquarters.roles["base guard"] = number_drones
        else:
...
class DevastatorDrone(Drone):
    ...
    def move_to_step(self, object):
        distance = min(randint(100, 250), max(50, self.distance_to(object) - 50))
    ...
    def shoot(self, object):
        ...
        def nearest_angle_distance(left, right):
            ...
            return normalise_angle(math.atan2(abs(rv.y - lv.y), abs(rv.x - lv.x)) * 180 / math.pi)
        ...
        for partner in self.headquarters.soldiers:
            ...
            vector = Vector.from_points(self.coord, partner.coord, module=self.attack_range)

            if (self.distance_to(partner) < 50
                and abs(nearest_angle_distance(vector.direction, self.direction)) < 40):

                if self.cost_forpost < 5:
                    self.actions.append(["move to", choice(self.asteroids), 1])
                return
    ...
    def on_born(self):
        ...
        if self.have_gun:
            self.actions.append(["move", nearesst_aster[idx][1], 1])
...
class Defender(Behavior):
    ...
    def next_step(self, purpose):
        soldier = self.unit
        self.victim = purpose
        if soldier.distance_to(purpose) > soldier.attack_range or soldier.distance_to(soldier.my_mothership) < 20:
            soldier.actions.append(['move to', purpose.coord, 1])
...
class Spy(Defender):
    ...
    def next_step(self, purpose):
        ...
        if soldier.distance_to(purpose) > soldier.attack_range:
            soldier.actions.append(['move to', purpose, 1])
        soldier.actions.append(['turn', purpose, 1])
        soldier.actions.append(['shoot', purpose, 1])
